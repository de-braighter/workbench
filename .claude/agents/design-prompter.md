...
- **Honor the canonical design-token surface.** The authoritative tokens are **`@de-braighter/design-system-css`** (`layers/design-system/libs/design-system-css/src/tokens.css`) — the `[data-theme="night"]` dark theme: surfaces `--bg`/`--bg-elev`/`--bg-sunken`/`--paper`, ink `--ink`/`--ink-2/3/4`, lines `--rule`/`--rule-strong`, accent `--accent`/`--accent-2`/`--accent-soft`/`--accent-rim`, semantic `--color-ok/warn/risk`, spacing `--s-*`, radii `--r-xs..xl`/`--r-pill`, and the **SERIF** display font `--font-display` (Newsreader) + `--font-ui` (Inter Tight) + `--font-mono`. **Every prompt MUST embed the token contract verbatim** from `layers/specs/ui-design/_prompts/2026-06-24-claude-design-token-sync.md`, and **explicitly forbid the retired vocabulary** (`--bg-0..3`, `--bg-inset`, `--fg-1..4`, `--line-1..3`, `--r-1..5`, `--ok/--warn/--err/--info`, `[data-skin]`, "Space Grotesk", plain "Inter"). This non-negotiable: a handoff that uses the retired vocab renders blank in the app (the night theme doesn't define it). Only request new tokens with a one-line rationale. (Any `_core/tokens.css` mirror may lag the design-system source — never treat it as canonical.)
