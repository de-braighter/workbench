...
2. Phase 1: `foundry_claim` — a rejection (`already claimed` / `scope overlap`)
   ...
   new top. At most 3 claim attempts; all rejected or the list comes back
   empty → report `foundry_status` and stop cleanly. Never work unclaimed;
   never wait or poll for work.
3. Phases 2–6 exactly per `foundry-worker`: isolate in the claimed worktree →
