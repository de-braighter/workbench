...
- `kids_football.club` gains `demo_mode BOOLEAN NOT NULL DEFAULT true`; `ClubSchema` grows from its slice-1 `{clubName}` placeholder to the persisted entity shape `{id, clubName, demoMode}` (the `id` addition is part of this change — the repo mints and returns it).
...
- **Demo-banner exemption (pinned, plan-wave clarification):** the charter D7 demo banner attaches to signed-in pack pages (`PackHostComponent`). The pre-auth wizard page is exempt — no tenant and no pack data exist yet, and its copy already self-identifies as demo ("invites are mocked in this prototype"); the post-onboarding landing pages sit inside the existing shell. **D3 naming note:** wizard-created tenants take free-form names (they are fresh demo tenants, not pre-registered partner fixtures, so D3's `demo_*` partner-naming convention does not apply); their demo posture is carried by `demo_mode = true` + the `CreateClubResponseSchema` `demoMode: z.literal(true)` wire pin.
...
export class MutableStubTenantRegistry implements TenantRegistry {
  ...
  /** Optional interface member — InMemoryTenantRegistry implements it; delegate it too. */
  getTenantStatus(tenantId: string): Promise<string | null> {
    return this.inner.getTenantStatus(tenantId);
  }
}
