...
- **Focus areas:** new `KF_DRILL_FOCUS_AREAS` const in contracts (the prototype's `P_FOCUS` list) drives the library filter chips + editor chips. `DrillSchema.focus` stays a free `z.string().min(1)` (wire-stable); if an existing drill carries an out-of-catalog focus, the editor renders it as an extra selected chip (degenerate-data tolerance).
...
- [ ] **Step 1: Write the failing tests.**
  ...
  - Sign-in spec: `selectUser` with a coach navigates to `[..., 'drills']`; with club-admin → `[..., 'admin', 'members']` (existing assertion stays).
- [ ] **Step 2: Run → FAIL.**
