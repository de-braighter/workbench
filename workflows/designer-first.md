...
<!-- governance:workflow-designer-first:start -->
Default flow:
...
<!-- governance:workflow-designer-first:end -->

> ⚙️ The region above is **generated by the foundry** from the `workflow-designer-first` governance node (the markdown `CompileTarget`). Edit the node's `authoredContent` via the Studio `/governance` cockpit, not this file; the drift tripwire (`domains/foundry/src/compiler/__fixtures__/workflow-designer-first.md`) pins the foundry output. Part of the Unified Cockpit **D5.3** workflow-ownership tranche (design: `docs/superpowers/specs/2026-06-25-foundry-owns-workflows-tranche-design.md`).
