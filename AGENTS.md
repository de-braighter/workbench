...
1. The git repo `de-braighter/workbench` — canonical `.Codex/agents/`, `.Codex/skills/`, settings, policies, templates, workflows, project descriptors, and design docs.
2. The cluster root — sibling layer + domain + attic repos clone into `layers/`, `domains/`, and `attic/`.

**Codex is always launched from this directory.** `.Codex/` here applies to all work across the cluster. Do not launch Codex from inside a sibling repo (you would lose access to the agents and skills).
...
- Don't launch Codex from inside `layers/*` or `domains/*` — launch from `de-braighter/` root.
