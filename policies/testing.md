...
<!-- governance:policy-testing:start -->
## Principles
...
<!-- governance:policy-testing:end -->

> ⚙️ The region above is **generated by the foundry** from the `policy-testing` governance node (the markdown `CompileTarget`). Edit the node's `authoredContent` via the Studio `/governance` cockpit, not this file; the drift tripwire (`domains/foundry/src/compiler/__fixtures__/policy-testing.md`) pins the foundry output. Part of the Unified Cockpit **D5.2** policy-ownership tranche (design: `docs/superpowers/specs/2026-06-25-foundry-owns-policies-tranche-design.md`).
