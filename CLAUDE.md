...
- **Feed the SDLC twin on PRs (any repo)** — give `pack-devloop` the signal it can't get from `gh`, by convention in the PR body + a per-PR ritual:
  ...
  - **`Effect:` line(s)** — when you can make a *defensible* prediction about a measurable delivery indicator, add `Effect: <indicatorId> <predicted>±<sd> [basis]` (e.g. `Effect: maintainability -1±0.5 expert`). Optional + non-gating (a calibratable claim, never a merge gate). **Prefer the auto-observable indicators** — `cycle-time`, `findings` (self-observing) and the SonarQube metrics `coverage`, `maintainability`, `security`, `reliability`, `smells`, `bugs`, `tech-debt`, `duplication`, `complexity`: `reconcile` observes these at merge, so the declaration gets scored with no manual `observe-effect`. **This is the dogfooding lever** — per-producer calibration stays `unknown` until PRs routinely carry *both* `Producer:` and `Effect:` (the calibration loop is built — devloop#2 — but starved of attributed data until then). Declare only what you can defend; the proper score rewards sharpness and penalises vague "always-covered" claims, so **omit rather than guess** for abstract/unobservable indicators (over-declaring scores *worse*, not better).
  - **The ritual** (from `domains/devloop`): after a verifier wave, `npm run dev -- drain <repo#pr>` (PR-scopes the verdicts so `findings`/`qa.score` score); after merge, `… backfill` then `… reconcile` (parses the body lines + observes effects); per the retro cadence, `… retro '{…}'` for notable PRs. See `policies/git.md`.
