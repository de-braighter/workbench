...
3. Derive, before claiming:
   ...
   - **worktree** — `<repo-local-path>/.Codex/worktrees/<slug>`
...
grep -q '\.Codex/worktrees/' .gitignore .git/info/exclude 2>/dev/null || echo '.Codex/worktrees/' >> .git/info/exclude
git worktree add .Codex/worktrees/<slug> -b feat/<slug> origin/main
cd .Codex/worktrees/<slug>
...
```bash
REPO_ABS=$(cd <repo-local-path> && pwd)   # ABSOLUTE — the CLI returns join(REPO_ABS, .Codex/wt-pool/slot-N), absolute
# exclude the pool dir in the TARGET repo (domains/foundry already gitignores it; OTHER repos need this):
grep -q '\.Codex/wt-pool/' "$REPO_ABS/.gitignore" "$REPO_ABS/.git/info/exclude" 2>/dev/null \
  || echo '.Codex/wt-pool/' >> "$REPO_ABS/.git/info/exclude"
...
pristine tree). Only a *cold* `git worktree add` at `.Codex/worktrees/<slug>` gets removed in
Phase 5 cleanup; a leased `.Codex/wt-pool/slot-N` is left in place for the next worker.
...
> harmless, the physical `<repo>/.Codex/wt-pool/slot-N` paths differ).
...
it. `/tech-debt` expects a per-repo `.Codex/sdlc.json` that may be absent, so it
...
3. Cleanup from the repo root — **only if you cold-added**: `git worktree remove
   .Codex/worktrees/<slug>` (it should be clean after a merge — investigate
   before reaching for `--force`), delete the merged branch. **If you leased a
   warm slot (`.Codex/wt-pool/slot-N`), leave it in place** — the pool reuses it
